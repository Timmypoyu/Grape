  ...
  let build_function_body fdecl =
    ...
    let lookup locals n = try StringMap.find n local_vars
      with Not_found -> 
        ...
        with Not_found ->
          try StringMap.find n locals
          with Not_found -> 
            raise (Failure ("Undeclared identifier " ^ n))
    in
    ...
    | SId s       -> L.build_load (lookup locals s) s builder
    | SProp (o, p) ->
      let tobj = fst o in
    ...
    | SAssign (s, e) -> let e' = expr locals builder e in
      ignore(L.build_store e' (lookup locals s) builder); e'
    ...
    | SListLit i ->
      ...
      let data_ptr = (match atyp with
        A.List _ | A.Graph (_,_) | A.Edge _ | A.Node _  -> expr locals builder sx 
    ...
    | SCall ("print", [e]) | SCall ("printb", [e]) ->
        L.build_call printf [| int_format_str ; (expr locals builder e) |] "printf" builder
    | SCall ("printbig", [e]) ->
        L.build_call printbig [| (expr locals builder e) |] "printbig" builder
    | SCall ("printf", [e]) -> 
        L.build_call printf [| float_format_str ; (expr locals builder e) |] "printf" builder
    | SCall ("prints", [e]) ->
        L.build_call printf [| str_format_str ; (expr locals builder e) |] "prints" builder
    | SCall ("size", [e]) -> 
        L.build_call size [|expr locals builder e|] "size" builder  
    | SCall ("str_size", [e]) -> 
        L.build_call str_size [|expr locals builder e|] "str_size" builder  
    | SCall (f, args) ->
    ...
    | SMethod (o, m, args) ->
      ...
      (match (tobj, m) with
        (A.Graph (n, e), "outgoing") ->
          let a' = expr locals builder (List.hd args) in
      ...
      | SDeclare(t, i, (atyp, a)) :: ss -> 
          ...
          let new_locals = (match i with
            | _ when StringMap.mem i locals ->
                ...
                StringMap.add i local clean_locals
            | _ -> 
                StringMap.add i local locals) in
          List.fold_left (stmt new_locals) builder ss
      | s :: ss -> 
          List.fold_left (stmt locals) builder ss
      in build_block locals sl
  | SExpr e -> ignore(expr locals builder e); builder
  | SDeclare (_, _, a) -> ignore(expr locals builder a); builder 
